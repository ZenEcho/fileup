  ...
  <div class="container-fluid">
    <section class="wrapperBox" v-if="isCountdownFinished" :class="{
      ...
      <div class="box" v-for="(item, index) in RenderingData.slice(0, 6)" :key="index">
        ...
        <span class="IsIcons">
          <span v-show="item.CDN === 1" class="isCDN" data-bs-toggle="tooltip" title="CDN">
          ...
          <span v-show="item.region !== 'CN'" class="isCN" data-bs-toggle="tooltip" title="海外"><svg t="1690365276304"
              class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2827" width="32"
              height="32">
              <path
...
export default {
  ...
  methods: {
    ...
    PageRendering(array) {
      array.forEach(item => {
        if (item.image.includes('?cache=')) {
          item.image = item.image.replace(/\?cache=\d+/, `?cache=${Date.now()}`);
        } else {
